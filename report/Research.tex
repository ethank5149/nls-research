...
\begin{document}
\title{\textbf{Multi-Peak Ground-State Solutions for the Nonlinear Schrödinger
...
nonlinear Schrödinger equation $(-\Delta+V+E)\psi+\sigma|\psi|^{2p}\psi=0$
...
triangles requiring $|\lambda_{2}|/|\lambda_{1}|\geq3$; and equilateral
triangles with a continuous rotational degree of freedom $\theta$,
admissible for all orientations when $\tfrac{1}{3}\leq\mu\leq3$ (where
$\mu=|\lambda_{2}|/|\lambda_{1}|$) and for restricted angular windows
otherwise. To extend these to finite energy, we construct explicit
solutions of the perturbation system $F_{i}(\delta y,R)=0$ at $R=0$,
identify the four-dimensional kernel of the linearization, and show
that gauge-fixing constraints reduce the problem to a system amenable
to the implicit function theorem.

\medskip{}
\textbf{Keywords:} nonlinear Schrödinger equation, multi-peak solutions,
concentration compactness, Lyapunov--Schmidt reduction, ground states,
optical fibers

\medskip{}
\noindent\textbf{GRANT PLACEHOLDER} 
...
to second order in frequency yields the nonlinear Schrödinger equation
(NLS): 
\[
...
\]
is mathematically identical to the stationary Schrödinger equation
...
\begin{figure}[htbp]
\centering \includegraphics[width=1\linewidth]{/run/user/1000/gvfs/smb-share:server=skynet.local,share=public/nls-research/figures/output/figure_7_methodology_flowchart}
\caption{Analytical pipeline from the PDE to multi-peak ground states. The
left column shows the framework established in Kirr--Natarajan~\cite{KirrNatarajan2018}:
rescaling, concentration compactness decomposition, and Lyapunov--Schmidt
reduction to a finite-dimensional algebraic system. The right column
shows the contributions of the present work: classification of admissible
peak geometries, construction of the perturbation system $F_{i}(\delta y,R)=0$,
kernel analysis, and gauge-fixing strategy.}
\label{fig:pipeline}
\end{figure}
...
obstruction in the reduced system.
...
\begin{figure}[htbp]
\centering \includegraphics[width=1\linewidth]{/run/user/1000/gvfs/smb-share:server=skynet.local,share=public/nls-research/figures/output/figure_4_profile_decomposition}
\caption{Multi-peak profile decomposition. The rescaled solution $u_{E}$ (solid
black) is approximated by a sum of well-separated profiles $u_{R}(\cdot-z_{i})$
(colored dashed), each solving the translation-invariant NLS~(\ref{eq:profile}).
The remainder $h_{R}$ (gray shaded) satisfies $\norm{h_{R}}_{H^{1}}\to0$
as $R\to0$.}
...
\end{prop}
\begin{figure}[htbp]
\centering \includegraphics[width=1\linewidth]{/run/user/1000/gvfs/smb-share:server=skynet.local,share=public/nls-research/figures/output/figure_5_concentration_vs_peaks}
\caption{Concentration of rescaled peak positions. As $E\to\infty$ (equivalently
$R\to0$), the rescaled positions $Rz_{i}$ converge to the critical
point $x_{0}$ of $V$. The peak positions $z_{i}$ in the blown-up
coordinates separate to distances $\sim m_{R}$, the minimum inter-peak
spacing.}
...
\end{defn}
\begin{figure}[htbp]
\centering \includegraphics[width=1\linewidth]{/run/user/1000/gvfs/smb-share:server=skynet.local,share=public/nls-research/figures/output/figure_6_interaction_decay}
\caption{Left: Exponential decay of the interaction functional $Q_{R}(d)=-\sigma\langle\partial_{\hat{d}}\,u_{R}(\cdot-d),\,u_{R}^{2p+1}\rangle$
as a function of inter-peak distance $d$, illustrating that only
nearest-neighbor interactions contribute at leading order. Right:
The interaction coefficient $\alpha_{ki}=Q_{R}(|z_{k}-z_{i}|)/Q_{R}(m_{R})$
as a function of the relative distance $|z_{k}-z_{i}|/m_{R}$, normalized
to unity at the minimum distance.}
...
limit $R\to0$ with $y_{i}=\lim_{R\to0}s_{i}/(Rm_{R})$: 
\begin{equation}
...
\begin{figure}[htbp]
\centering \includegraphics[width=1\linewidth]{/run/user/1000/gvfs/smb-share:server=skynet.local,share=public/nls-research/figures/output/figure_2_peak_configurations}
\caption{Representative peak configurations classified in Theorem~\ref{thm:classification}.
From left to right: collinear arrangements with $M=2$ and $M=5$
peaks along a single eigenvector; isosceles triangle ($M=3$) requiring
$\lambda_{2}=3\lambda_{1}$; equilateral triangle ($M=3$) requiring
$|\lambda_{2}|/|\lambda_{1}|\protect\geq3$; and equilateral triangle
with rotational degree of freedom $\theta$, with a schematic perturbation
$\delta y$.}
...
with $\alpha_{12}=\alpha_{13}=-\lambda_{2}/3$, $\alpha_{23}=\lambda_{2}/6-\lambda_{1}/2$,
requiring $|\lambda_{2}|/|\lambda_{1}|\geq3$. 
...
The admissible $(\mu,\theta)$ regions, where $\mu=|\lambda_{2}|/|\lambda_{1}|$,
...
Theorem~3.1 that follow, the algebraic system~(2.4) is written in
the absorbed form 
\begin{equation}
...
from Definition~2.8. This convention avoids carrying $\chi$ through
the algebraic manipulations.

\smallskip{}
To recover the normalized coefficients of Definition~2.7 (satisfying
$0\leq\alpha_{ki}\leq1$ with $\alpha_{ki}=1$ at minimum distance),
one divides by $\chi$: 
\[
...
below yield $\hat{\alpha}_{12}=\hat{\alpha}_{13}=-\lambda_{2}/3$
and $\hat{\alpha}_{23}=\lambda_{2}/6-\lambda_{1}/2$. Since $\hat{\alpha}_{12}=\hat{\alpha}_{13}\geq\hat{\alpha}_{23}$
...
\[
\alpha_{23}=\frac{\hat{\alpha}_{23}}{\chi}=\frac{\lambda_{2}/6-\lambda_{1}/2}{-\lambda_{2}/3}=1-\frac{3}{2\mu}\;\in\;[0,1).
\]
In particular, $\alpha_{23}<1$ for $\mu>3$, reflecting the fact
that the base edge is \emph{not} at the minimum inter-peak distance:
the perturbation $\delta y^{0}$ of Proposition~4.1 pushes the base
vertices slightly apart relative to the apex--base edges. \end{rmk}
...
Let $y_{1}=(0,h)^{\intercal}$, $y_{2}=(-a,-h/2)^{\intercal}$, $y_{3}=(a,-h/2)^{\intercal}$
with $2a>1$ (so $|y_{2}-y_{3}|=2a>1$ and the nearest-neighbor pairs
are $(1,2)$ and $(1,3)$ only) and the geometric constraint $a^{2}+(3h/2)^{2}=1$.
...
Set $2a=1$, giving $a=1/2$, $h=1/\sqrt{3}$, and $|y_{k}-y_{i}|=1$
for all pairs. All three pairs are nearest neighbors. From $y_{1}$:
$\alpha_{12}=\alpha_{13}=-\lambda_{2}/3$ as before. From $y_{2}$
(with both $y_{1}$ and $y_{3}$ as neighbors): 
\begin{align*}
...
\end{proof}
\begin{figure}[htbp]
\centering \includegraphics[width=1\linewidth]{/run/user/1000/gvfs/smb-share:server=skynet.local,share=public/nls-research/figures/output/figure_3_alpha_vs_theta_vs_mu}
\caption{Interaction coefficients $\alpha_{12}(\theta)$, $\alpha_{13}(\theta)$,
$\alpha_{23}(\theta)$ as functions of the rotation angle $\theta\in[0,\pi/2]$
for four representative eigenvalue ratios $\mu\in\{0.3,\,1.0,\,3.0,\,6.0\}$.
A configuration is admissible at angle $\theta$ if and only if all
three coefficients are non-negative. For $\mu=1$ (isotropic potential)
all orientations are admissible. For $\mu=6>3$, the admissible $\theta$-range
breaks into two disjoint intervals.}
...
The constraints $\alpha_{12},\alpha_{13},\alpha_{23}\geq0$ with $\mu=|\lambda_{2}|/|\lambda_{1}|$
...
\end{prop}
\begin{table}[htbp]
\centering \caption{Summary of admissible $\theta$-ranges as a function of $\mu=|\lambda_{2}|/|\lambda_{1}|$.}
\label{tab:summary} %
\begin{tabular}{cc}
\toprule 
...
{[}8pt{]} $\frac{1}{3}\leq\mu\leq3$ & $\bigl[0,\;\frac{\pi}{2}\bigr]$\tabularnewline
{[}8pt{]} $\mu>3$ & $\bigl[\frac{\varphi(\mu)}{2},\;\frac{\pi}{3}-\frac{\varphi(\mu)}{2}\bigr]\cup\bigl[\frac{\pi}{3}+\frac{\varphi(\mu)}{2},\;\frac{\pi}{2}\bigr]$\tabularnewline
\bottomrule
\end{tabular}
\end{table}

\begin{figure}[htbp]
\centering \includegraphics[width=0.75\linewidth]{/run/user/1000/gvfs/smb-share:server=skynet.local,share=public/nls-research/figures/output/figure_1_admissibility_binding}
\caption{Admissible region (shaded) in the $(\mu,\theta)$ parameter space
for equilateral triangle configurations with rotational degree of
freedom. The dashed vertical lines mark $\mu=1/3$ and $\mu=3$, the
boundaries of the fully admissible strip. For $\mu$ outside this
strip, the constraint $\alpha_{ki}\protect\geq0$ excludes angular
windows whose width is controlled by $\varphi(\mu)=\arccos(\nu(\mu))$.
The boundary curves correspond to the binding constraints where exactly
one $\alpha_{ki}=0$.}
...
\label{prop:explicit-soln} Define $\beta=\frac{1}{2}\ln(\alpha_{12}/\alpha_{23})$
and $\gamma=\frac{2\beta}{3\sqrt{3}}$, where $\alpha_{12}=-\lambda_{2}/3$
and $\alpha_{23}=\lambda_{2}/6-\lambda_{1}/2$ are the unperturbed
coefficients. Then 
\begin{equation}
...
second uses $\ip{y_{3}-y_{2}}{\delta y_{3}^{0}-\delta y_{2}^{0}}=2\beta=\ln(\alpha_{12}/\alpha_{23})$.

Setting $\chi=-\lambda_{2}/3$ (the overall interaction strength from
Remark~\ref{rmk:alpha-convention}, equal to the combined force coefficient
$\hat{\alpha}_{12}$ of the apex--base edges), the system $F_{i}(\delta y^{0},0)=0$
becomes 
\[
...
\begin{align}
D_{\delta y}P_{\perp}F(\delta y^{0},0)[\delta w] & =P_{\perp}\sum_{k\in N_{i}}\ip{\delta w_{k}-\delta w_{i}}{y_{k}-y_{i}}\,\tilde{\alpha}_{ik}(0)\,\chi\,(y_{k}-y_{i}),\label{eq:DPperp}\\
D_{\delta y}P_{w}F(\delta y^{0},0)[\delta w] & =\sum_{i}\Bigl[\ip{w_{i}}{\HV\delta w_{i}}+\sum_{k\in N_{i}}\tilde{\alpha}_{ki}(0)\bigl(\ip{w_{i}}{\delta y_{k}^{0}-\delta y_{i}^{0}}\ip{y_{k}-y_{i}}{\delta w_{k}-\delta w_{i}}\nonumber \\
 & \quad\qquad-\ip{w_{i}}{\delta w_{k}-\delta w_{i}}\bigr)\Bigr].\label{eq:DPpar}
\end{align}
...
each $\theta$, with $\beta(\theta)=\frac{1}{2}\ln(\alpha_{12}(\theta)/\alpha_{23}(\theta))$
and $\gamma(\theta)=2\beta(\theta)/(3\sqrt{3})$. The resulting $\delta y^{0}(\theta)$
is smooth in $\theta$ since the $\alpha_{ki}(\theta)$ are smooth
and positive in the admissible range. 
\end{proof}
\begin{cor}
\label{cor:tangent-kernel} The tangent vector $\frac{d}{d\theta}\delta y^{0}(\theta)$
...
of Proposition~4.1 satisfies this condition, and the gauge-fixing
constraints \textup{(G1)--(G2)} are compatible with~$\delta y^{0}$. 
\end{prop}
\begin{proof}
The interaction terms in $F_{i}$ (Definition~4.1) involve differences
$(y_{k}-y_{i})$ and $(\delta y_{k}-\delta y_{i})/\tilde{m}_{R}$.
...
ground states for the nonlinear Schrödinger equation 
\[
...
\begin{table}[H]
\centering
\begin{tabularx}{\columnwidth}{>{\centering\arraybackslash}Xc}
\toprule 
Symbol & Description\tabularnewline
\midrule
$\psi:\mathbb{R}^{n}\to\mathbb{C}$ & Solution of the nonlinear Schrödinger equation\tabularnewline
...
Symbol & Description\tabularnewline
\midrule
$R=E^{-\frac{1}{2}}$ & Scaling parameter; $R\to0$ as $E\to\infty$\tabularnewline
$u_{E}\left(x\right)=E^{-\frac{1}{2p}}\psi_{E}\left(E^{-\frac{1}{2}}x\right)$ & Rescaled solution with $\left\Vert u_{E}\right\Vert _{H^{1}}$ uniformly
...
\begin{table}[H]
\centering
\begin{tabularx}{\columnwidth}{c>{\centering\arraybackslash}X}
\toprule 
Symbol & Description\tabularnewline
\midrule
...
Symbol & Description\tabularnewline
\midrule
$Q_{R}\left(z\right)$ & Interaction functional: $Q_{R}\left(z\right)=-\sigma\left\langle \partial_{\frac{z}{\left|z\right|}}u_{R}\left(\cdot-z\right),u_{R}^{2p+1}\right\rangle $\tabularnewline
...
\end{cellvarwidth}\tabularnewline
$\varphi\left(\mu\right)=\arccos\left(\nu\left(\mu\right)\right)$ & Critical angle function: $\varphi=\frac{\pi}{3}$ when $\nu\leq\frac{1}{2}$,
...
Symbol & Description\tabularnewline
\midrule
$D_{\delta y}F_{i}$ & Fréchet derivative of $F_{i}$ with respect to $\delta y$\tabularnewline
...
Symbol & Description\tabularnewline
\midrule 
$\psi_{E}$ & Solution of~(\ref{eq:NLS}) at energy $E$\tabularnewline
$V$, $\HV(x_{0})$ & External potential and its Hessian at $x_{0}$\tabularnewline
$\lambda_{i}$, $\boldsymbol{r}_{i}$ & Eigenvalues/eigenvectors of $\HV(x_{0})$; $\lambda_{i}<0$\tabularnewline
$\mu=|\lambda_{2}|/|\lambda_{1}|$ & Eigenvalue ratio\tabularnewline
$R=E^{-1/2}$ & Rescaling parameter; $R\to0$ as $E\to\infty$\tabularnewline
$u_{E}(x)=E^{-1/(2p)}\psi_{E}(E^{-1/2}x)$ & Rescaled solution\tabularnewline
...
$m_{R}$ & Minimum inter-peak distance\tabularnewline
$y_{i}=\lim s_{i}/(Rm_{R})$ & Limiting peak positions\tabularnewline
$\delta y_{i}$ & Perturbation of peak $i$; $\delta y_{i}\in\operatorname{span}\{\boldsymbol{r}_{1},\boldsymbol{r}_{2}\}$\tabularnewline
$F_{i}(\delta y,R)$ & Perturbation system (Definition~\ref{def:Fi})\tabularnewline
$\tilde{\alpha}_{ki}=e^{-\left\langle y_{k}-y_{i},\delta y_{k}-\delta y_{i}\right\rangle }$ & Perturbed interaction coefficient\tabularnewline
...
\caption{Notation - Principal Notation}
\end{table}
\par\end{center}
