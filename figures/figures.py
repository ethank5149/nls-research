...
def compute_alphas(mu, theta):
    ...
    a12 = (1 / 3) * (lam2 * (-np.sqrt(3) * s - c) * c + lam1 * (-s + np.sqrt(3) * c) * s)
    a13 = (1 / 3) * (lam2 * (np.sqrt(3) * s - c) * c - lam1 * (s + np.sqrt(3) * c) * s)
    a23 = (1 / 6) * (lam2 * (2 * np.cos(2 * theta) - 1) - lam1 * (2 * np.cos(2 * theta) + 1))
    return a12, a13, a23


def box(ax, x, y, w, h, text, fc, ec, fs=9, bold=False, lw=1.5, ls="-"):
    """Draw a rounded box with centered text."""
    b = FancyBboxPatch((x - w / 2, y - h / 2), w, h, boxstyle="round,pad=0.15", facecolor=fc, edgecolor=ec, lw=lw, linestyle=ls)
    ax.add_patch(b)
    ax.text(x, y, text, ha="center", va="center", fontsize=fs, fontweight="bold" if bold else "normal", multialignment="center")


def arr(ax, x1, y1, x2, y2, color="#2c3e50", lw=1.5, ls="-"):
    """Draw an arrow annotation."""
    ax.annotate("", xy=(x2, y2), xytext=(x1, y1), arrowprops=dict(arrowstyle="->", color=color, lw=lw, linestyle=ls, shrinkA=1, shrinkB=1))


def draw_peak(ax, pos, label, color="#2c3e50", ms=9):
    ax.plot(*pos, "o", color=color, ms=ms, zorder=5)
    ax.annotate(label, xy=pos, xytext=(pos[0] + 0.06, pos[1] + 0.12), fontsize=10, color=color, ha="center",)
...
def draw_edge(ax, p1, p2, alpha_val=None, color="#7f8c8d", lw=1.3):
    ...
    if alpha_val is not None:
        ...
        ax.text(mx + nx, my + ny, alpha_val, fontsize=7, ha="center", va="center", color="#2980b9", fontweight="bold", bbox=dict(boxstyle="round,pad=0.15", facecolor="white", edgecolor="#ddd", alpha=0.9),)


def draw_eigvec(ax, origin, direction, label, color="#c0392b"):
    ax.annotate("", xy=(origin[0] + direction[0], origin[1] + direction[1]), xytext=origin, arrowprops=dict(arrowstyle="->", color=color, lw=1.3))
    ax.text(origin[0] + direction[0] * 1.12, origin[1] + direction[1] * 1.12, label, fontsize=9, color=color, ha="center", va="center")


def save_figure(fig, output_dir, stem, *, pad_inches=0.15):
    """Save figure in PNG, PDF, and EPS formats."""
    for ext in ("png",):  # "pdf", "eps"):
        fig.savefig(output_dir / f"{stem}.{ext}", bbox_inches="tight", pad_inches=pad_inches)
    plt.close(fig)


def create_figure_1():
    """Figure 1 — Admissibility & Binding Constraint (μ,θ) Phase Diagram."""
    print("Creating Figure 1 - Admissibility and Binding Constraints...", end=" ", flush=True)
    ...
    ax.pcolormesh(MU, TH, binding_masked, cmap=cmap, vmin=-0.5, vmax=3.5, shading="auto")

    ax.contour(MU, TH, admissible.astype(float), levels=[0.5], colors="#1a1a2e", linewidths=2.0)
    ...
    for val, label, col, bg in [
        ...
        ax.text(
            val + 0.12, label_y, label,
            ha="left", fontsize=9, color=col, fontweight="bold",
            bbox=dict(boxstyle="round,pad=0.12", facecolor=bg, edgecolor="none", alpha=0.8),
        )

    ax.text(
        1.65, np.pi / 4,
        "All $\\theta$\nadmissible",
        fontsize=9, ha="center", color="white", fontweight="bold", style="italic",
        bbox=dict(boxstyle="round,pad=0.3", facecolor="#333", edgecolor="none", alpha=0.45),
    )

    ax.plot(3, 0, "o", color="white", ms=6, mec="#1a1a2e", mew=1.5, zorder=5)
    ax.annotate(
        r"$\lambda_2 = 3\lambda_1$",
        xy=(3, 0), xytext=(4.2, np.pi / 6),
    ...
    ax.set_yticklabels([r"$0$", r"$\frac{\pi}{6}$", r"$\frac{\pi}{3}$", r"$\frac{\pi}{2}$"])

    legend_elements = [
        mpatches.Patch(color="#e74c3c", label=r"$\alpha_{12}$ binding (apex $\leftrightarrow$ base-left)"),
        mpatches.Patch(color="#e67e22", label=r"$\alpha_{13}$ binding (apex $\leftrightarrow$ base-right)"),
        mpatches.Patch(color="#7f8c8d", label=r"$\alpha_{23}$ binding (base edge)"),
        mpatches.Patch(color="#ececec", label=r"Inadmissible ($\exists\,\alpha_{ij} < 0$)"),
        Line2D([0], [0], color="#1a1a2e", lw=2, label="Admissibility boundary"),
    ]
    ax.legend(
        handles=legend_elements,
        loc="upper right", fontsize=7.5, framealpha=0.95, edgecolor="#ccc",
...
def create_figure_2():
    ...
    ax.annotate("", xy=(0.5, -0.22), xytext=(-0.5, -0.22),
    ...
    ax.set_xlim(-1.1, 1.1); ax.set_ylim(-0.8, 0.5)
    ...
    ax.set_xlim(-1.5, 1.5); ax.set_ylim(-0.8, 0.5)
    ax.set_aspect("equal"); ax.axis("off")
    ...
    y1 = (0, h_iso); y2 = (-a_iso, -h_iso / 2); y3 = (a_iso, -h_iso / 2)
    for p1, p2, albl in [(y1, y2, r"$\alpha_{12}$"), (y1, y3, r"$\alpha_{13}$"),
                         (y2, y3, r"$\alpha_{23}=0$")]:
        c_edge = "#e74c3c" if "=0" in albl else "#7f8c8d"
        draw_edge(ax, p1, p2, albl, color=c_edge)
    draw_peak(ax, y1, r"$y_1$"); draw_peak(ax, y2, r"$y_2$"); draw_peak(ax, y3, r"$y_3$")
    ...
    ax.set_xlim(-1.1, 1.1); ax.set_ylim(-1.1, 0.85)
    ax.set_aspect("equal"); ax.axis("off")
    ax.set_title(r"(d) Equilateral, $\theta = 0$", fontsize=11, pad=10)

    # ── (e) Equilateral with perturbation δy⁰ ── (FIXED LaTeX)
    ax = ax_e
    y1_arr = np.array([0, h_eq]); y2_arr = np.array([-a_eq, -h_eq / 2]); y3_arr = np.array([a_eq, -h_eq / 2])
    mu_val = 4.0; lam1 = -1.0; lam2 = -mu_val
    alpha12 = -lam2 / 3; alpha23_val = (-lam1 + lam2 / 3) / 2
    beta = 0.5 * np.log(alpha12 / alpha23_val)
    gamma = 2 * beta / (3 * np.sqrt(3))
    r1 = np.array([1, 0]); r2 = np.array([0, 1])
    dy1 = -gamma * r2; dy2 = -beta * r1 + (gamma / 2) * r2; dy3 = beta * r1 + (gamma / 2) * r2
    sc = 0.7
    ...
    for orig, pert, col in [(y1_arr, py1, "#e74c3c"), (y2_arr, py2, "#27ae60"), (y3_arr, py3, "#2980b9")]:
        ax.annotate("", xy=pert, xytext=orig, arrowprops=dict(arrowstyle="->", color=col, lw=1.3))
    ...
    ax.set_xlim(-1.0, 1.0); ax.set_ylim(-1.15, 0.85)
    ax.set_aspect("equal"); ax.axis("off")
    ax.set_title(r"(e) Perturbation $\delta y^0$ at $R=0$, $\mu=4$", fontsize=11, pad=10)
...
def create_figure_3():
    ...
    print("Creating Figure 3 - α vs θ for representative μ values...", end=" ", flush=True)
    ...
    for mu, col in zip(mu_vals, mu_colors):
        ...
        for data, key in [(a12, "a12"), (a13, "a13"), (a23, "a23")]:
            ax.plot(np.degrees(theta), data, ls=styles[key][0], lw=styles[key][1], color=col, zorder=3)

    # Forbidden label — positioned to avoid curve overlap
    ax.text(45, -0.35, r"$\alpha < 0$: forbidden (focusing nonlinearity requires attractive interaction)", fontsize=8, ha="center", color="#c0392b", style="italic", bbox=dict(boxstyle="round,pad=0.15", facecolor="white", edgecolor="#fadbd8", alpha=0.9))
    ...
    coeff_handles = [
        Line2D([0], [0], color="#333", ls="-", lw=2.0, label=r"$\alpha_{12}$  (apex $\leftrightarrow$ base-left)"),
        Line2D([0], [0], color="#333", ls="--", lw=1.8, label=r"$\alpha_{13}$  (apex $\leftrightarrow$ base-right)"),
        Line2D([0], [0], color="#333", ls=":", lw=2.2, label=r"$\alpha_{23}$  (base edge)")
        ]
    mu_handles = [Line2D([0], [0], color=col, ls="-", lw=3, label=rf"$\mu = {mu}$") for mu, col in zip(mu_vals, mu_colors)]
    leg1 = ax.legend(handles=coeff_handles, loc="upper left", fontsize=8, title="Coefficient (linestyle)", title_fontsize=8, framealpha=0.95, edgecolor="#ccc", borderpad=0.7)
    ax.add_artist(leg1)
    ax.legend(handles=mu_handles, loc="upper right", fontsize=8, title=r"$\mu = |\lambda_2|/|\lambda_1|$ (color)", title_fontsize=8, framealpha=0.95, edgecolor="#ccc", borderpad=0.7)
    ax.set_title(r"Interaction Coefficients $\alpha_{12}$, $\alpha_{13}$, $\alpha_{23}$ vs Orientation $\theta$", fontsize=13, pad=12)
...
def create_figure_4():
    ...
    print("Creating Figure 4 - Profile Decomposition Illustration...", end=" ", flush=True)
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(9, 6), height_ratios=[3, 1], gridspec_kw={"hspace": 0.32})

    ax1.fill_between(x, 0, u_sum, color="#4a90d9", alpha=0.12)
    ax1.plot(x, u_sum, color="#1a3a6e", lw=2.2,
    ...
    for zi, lab, col in [(z1, r"$z_1$", "#e74c3c"), (z2, r"$z_2$", "#27ae60"), (z3, r"$z_3$", "#f39c12")]:
    ...
    ax1.annotate("", xy=(z2, 0.42), xytext=(z1, 0.42),
                 arrowprops=dict(arrowstyle="<->", color="#555", lw=0.9))
    ax1.text((z1 + z2) / 2, 0.48, r"$m_R = \min_{i \neq j} |z_i - z_j|$", ha="center", fontsize=9, color="#555", bbox=dict(boxstyle="round,pad=0.15", facecolor="white", edgecolor="#ddd", alpha=0.85))

    ax1.text(0.02, 0.95, r"Each $u_R$ solves $(-\Delta + R^2 V(x_0) + 1)u + \sigma|u|^{2p}u = 0$", transform=ax1.transAxes, fontsize=8, va="top", color="#555", style="italic", bbox=dict(boxstyle="round,pad=0.2", facecolor="#fef9e7", edgecolor="#ddd"))

    ax1.set_ylabel(r"$u_E(x)$", labelpad=8)
    ax1.set_xlim(-14, 14); ax1.set_ylim(-0.18, 1.25)
    ax1.legend(loc="upper right", fontsize=9, framealpha=0.9, borderpad=0.7)
    ax1.set_title(r"Multi-Peak Decomposition: $u_E(x) \approx \sum_{i=1}^{M} u_R(x - z_i(R))$", fontsize=12, pad=10)
    ...
    ax2.set_xlim(-14, 14)
    ax2.set_title(r"Remainder $h_R$: $\|h_R\|_{H^1} \to 0$ as $E \to \infty$ (solved via IFT, Lemma 2)", fontsize=10, pad=8)
...
def create_figure_5():
    ...
    print("Creating Figure 5 - Concentration Points vs Peak Positions...", end=" ", flush=True)
    ...
    configs = [
        {"E": 1, "R": 1.0, "peaks": [(0, 1.0, 1.5)], "title": r"$E = 1$, $R = 1.0$", "label": "Single peak at $x_0$"},
        {"E": 4, "R": 0.5, "peaks": [(-1.5, 0.9, 0.8), (1.5, 0.9, 0.8)], "title": r"$E = 4$, $R = 0.5$", "label": "Peaks splitting from $x_0$"},
        {"E": 16, "R": 0.25, "peaks": [(-3.0, 1.0, 0.5), (3.0, 1.0, 0.5)], "title": r"$E = 16$, $R = 0.25$", "label": "Well-separated peaks"},
        {"E": 64, "R": 0.125, "peaks": [(-4.0, 1.0, 0.35), (0.0, 0.8, 0.35), (4.0, 1.0, 0.35)], "title": r"$E = 64$, $R = 0.125$", "label": "$M=3$ peaks"},
    ]
    ...
    for idx, (ax, cfg) in enumerate(zip(ax_profiles, configs)):
        ...
        ax.set_xlim(-8, 8); ax.set_ylim(-0.22, 1.3)
    ...
    ax_profiles[0].plot([], [], "v", color="#e74c3c", ms=7, label=r"Peak centers $z_i(E)$")
    ax_profiles[0].plot([], [], "s", color="#8e44ad", ms=5, label=r"Rescaled $Rz_i$")
    ax_profiles[0].plot([], [], "--", color="#27ae60", lw=1.2, label=r"Critical point $x_0$")
    ...
    ax_conv.semilogx(E_vals, Rz_1, "o-", color="#e74c3c", lw=1.5, ms=6, label=r"$Rz_1(E)$")
    ax_conv.semilogx(E_vals, Rz_2, "s-", color="#2980b9", lw=1.5, ms=6, label=r"$Rz_2(E)$")
    ax_conv.axhline(x0, color="#27ae60", ls="--", lw=1.5, label=r"$x_0$ (critical point)")
...
def create_figure_6():
    ...
    ax1.set_xlim(0.5, 12); ax1.set_ylim(1e-8, 1)
    ...
    ax1.text(
        8.5, 3e-7, "Negligible interaction\n" r"(justifies $N_i$ truncation)",
...
def create_figure_7():
    ...
    def box(ax, x, y, w, h, text, fc, ec, fs=7, bold=False, lw=1.2, ls="-"):
        b = FancyBboxPatch((x - w / 2, y - h / 2), w, h, boxstyle="round,pad=0.08", facecolor=fc, edgecolor=ec, lw=lw, linestyle=ls)
        ax.add_patch(b)
        ax.text(x, y, text, ha="center", va="center", fontsize=fs, fontweight="bold" if bold else "normal", multialignment="center", color="#1a1a1a", linespacing=1.25)

    def arr(ax, x1, y1, x2, y2, color=C_ARROW, lw=1.0, ls="-"):
        ax.annotate("", xy=(x2, y2), xytext=(x1, y1), arrowprops=dict(arrowstyle="->", color=color, lw=lw, linestyle=ls, shrinkA=1, shrinkB=1))

    def col_header(ax, x, y, w, text, color):
        rect = FancyBboxPatch((x - w / 2, y - 0.22), w, 0.44, boxstyle="round,pad=0.06", facecolor=color, edgecolor=color, lw=1.5, alpha=0.90)
        ax.add_patch(rect)
    ...
    box(ax, Lx, Y, Bw, bh, r"$(-\Delta + V + E)\psi + \sigma|\psi|^{2p}\psi = 0$"
        "\nFull PDE on "
        r"$\mathbb{R}^n$", C_BLUE_BG, C_BLUE_FG, fs=6.8, bold=True)

    Y = step(ax, Lx, Y, bh, bh)
    box(ax, Lx, Y, Bw, bh, r"Rescaling:  $u_E = E^{-1/(2p)}\psi_E(E^{-1/2}\cdot)$"
        "\n"
        r"$\|u_E\|_{H^1}$ bounded;  $R = E^{-1/2}\to 0$", C_GRAY_BG, C_GRAY_FG, fs=6.3)

    Y = step(ax, Lx, Y, bh, bh)
    box(ax, Lx, Y, Bw, bh, "Conc.-Compactness (Lions)"
        "\n"
        r"$u_E \approx \sum_{i=1}^{M} u_R(\cdot - z_i) + h_R$", C_GRAY_BG, C_GRAY_FG, fs=6.3)

    Y = step(ax, Lx, Y, bh, bh)
    box(ax, Lx, Y, Bw, bh, "Lyapunov\u2013Schmidt (Lemma 2)"
        "\n"
        r"$P_\perp F = 0 \Rightarrow h_R = h_R(z_1,\ldots,z_M,R)$", C_GRAY_BG, C_GRAY_FG, fs=6.3)

    Y = step(ax, Lx, Y, bh, bhL)
    box(ax, Lx, Y, Bw, bhL, "Reduced System"
        "\n"
        r"$H_V y_i = \sum_{k \in N_i} \alpha_{ki}\,\chi(y_k - y_i)$"
        "\n"
        r"peaks $\in$ unstable manifold of $H_V$", C_GREEN_BG, C_GREEN_FG, fs=6.3, bold=True)

    Y = step(ax, Lx, Y, bhL, bhL)
    box(ax, Lx, Y, Bw, bhL, "Thm 4.3: one profile per critical point"
        "\n"
        r"$\|U_E - \sum \mu_i u_\infty(\cdot - x_i\sqrt{E})\|_{H^2} < \varepsilon$"
        "\nExistence + uniqueness + stability", C_GREEN_BG, C_GREEN_FG, fs=6.3)

    Y = step(ax, Lx, Y, bhL, tag_h)
    box(ax, Lx, Y, Bw * 0.78, tag_h, "Resolved for distinct critical points", C_GREEN_BG, "#1a5632", fs=6.0, bold=True)

    Y = step(ax, Lx, Y, tag_h, bhL)
    box(ax, Lx, Y, Bw, bhL, "Open Problem (K-N Remark 4.1):"
        "\n"
        r"Multiple profiles $\to$ same critical point"
        "\n"
        r"along eigenvectors with $\lambda_j < 0$", C_RED_BG, C_RED_FG, fs=6.3, bold=True, lw=1.6)

    Y_open = Y
    yb_L = Y_open - bhL / 2 - 0.10
    yt_col = Y_top - 0.48

    left_rect = FancyBboxPatch((Lx - Bw / 2 - pad, yb_L), Bw + 2 * pad, yt_col - yb_L, boxstyle="round,pad=0.10", facecolor="none", edgecolor=C_BLUE_FG, lw=0.7, alpha=0.22)
    ...
    mid_y = (Y_open + Y_H) / 2
    arr(ax, Lx + Bw / 2 + 0.08, Y_open + 0.05, Rx - Bw / 2 - 0.08, Y_H - 0.05, color=C_RED_FG, lw=1.5)
    ...
    box(ax, Rx, Y, Bw, bh, r"Classify Limiting Configs ($R = 0$)"
        "\nCollinear | Isosceles | Equilateral | Rot. "
        r"$\theta$", C_GRAY_BG, C_GRAY_FG, fs=6.3)

    Y = step(ax, Rx, Y, bh, bh)
    box(ax, Rx, Y, Bw, bh, r"Admissibility:  $\alpha_{ki} \geq 0$"
        "\n"
        r"$\frac{1}{3} \leq \mu \leq 3$: all $\theta$;  restricted otherwise", C_GREEN_BG, C_GREEN_FG, fs=6.3)

    Y = step(ax, Rx, Y, bh, bh)
    box(ax, Rx, Y, Bw, bh, r"Perturbation System:  $F_i(\delta y, R) = 0$"
        "\n"
        r"Extended to $R = 0$ by continuity", C_BLUE_BG, C_BLUE_FG, fs=6.3)

    Y = step(ax, Rx, Y, bh, bhL)
    box(ax, Rx, Y, Bw, bhL, r"Explicit Solution at $R=0$ (Claim 3)"
        "\n"
        r"$\delta y_1^0=-\gamma\mathbf{r}_2$, "
        r"$\delta y_{2,3}^0=\mp\beta\mathbf{r}_1+\frac{\gamma}{2}\mathbf{r}_2$", C_BLUE_BG, C_BLUE_FG, fs=6.3)

    Y = step(ax, Rx, Y, bhL, bhL)
    box(ax, Rx, Y, Bw, bhL, "Kernel Obstruction (Claim 4)"
        "\n"
        r"$\Omega = \ker(D_{\delta y}F_i) \supseteq \{(v,v,v):v \in \mathbb{R}^2\}$"
        "\nIFT blocked by translational invariance", C_RED_BG, C_RED_FG, fs=6.3, bold=True, lw=1.6)
    ...
    box(ax, Rx, Y, Bw, bh, r"Gauge-Fixing:  $\delta y_1 \perp \mathbf{r}_1$  ;  "
        r"$(\delta y_3-\delta y_2) \parallel \mathbf{r}_1$", C_GRAY_BG, C_GRAY_FG, fs=6.3)

    Y = step(ax, Rx, Y, bh, bhL)
    box(ax, Rx, Y, Bw, bhL, "IFT + Lift via Lemma 2"
        "\n"
        r"$\Rightarrow$ exact multi-peak $\psi_E$ for $E \gg 1$"
        "\n(contingent on completing open steps)", C_GREEN_BG, "#1a5632", fs=6.3, bold=True, lw=1.3, ls="--")
    Y_Q = Y

    yb_R = Y_Q - bhL / 2 - 0.10
    right_rect = FancyBboxPatch((Rx - Bw / 2 - pad, yb_R), Bw + 2 * pad, yt_col - yb_R, boxstyle="round,pad=0.10", facecolor="none", edgecolor=C_HDR_R, lw=0.7, alpha=0.22)
    ax.add_patch(right_rect)

    # Open-steps callout (below right border, same as original)
    Y_os = yb_R - 0.42
    box(ax, Rx, Y_os, Bw, 0.55, "Remaining:  (1) Verify equivariance of "
        r"$F_i$"
        "\n(2) Invertibility on gauge-fixed subspace"
        "\n(3) Extend to continuous "
        r"$\theta$", C_OPEN_BG, C_OPEN_FG, fs=5.8, lw=1.0, ls="--")
    ...
    for i, (fc, ec, label) in enumerate([
            (C_BLUE_BG, C_BLUE_FG, "Systems"), (C_GRAY_BG, C_GRAY_FG, "Analysis"), (C_GREEN_BG, C_GREEN_FG, "Results"), (C_RED_BG, C_RED_FG, "Open"), ]
    ):
        xx = leg_x + i * dx
        swatch = FancyBboxPatch((xx, leg_y - 0.09), 0.26, 0.18, boxstyle="round,pad=0.03", facecolor=fc, edgecolor=ec, lw=0.7)
        ax.add_patch(swatch)
    ...
    for ext in ("png",):  # "pdf", "eps"):
        fig.savefig(OUTPUT_DIR / f"figure_7_methodology_flowchart.{ext}", bbox_inches="tight", pad_inches=0.08)
    plt.close(fig)
    print("Done!")


if __name__ == "__main__":
    figures = [create_figure_1, create_figure_2, create_figure_3, create_figure_4, create_figure_5, create_figure_6, create_figure_7]
